...
export class Home extends Component<any, HomeState> {
  ...
  public componentDidMount = () => {
    if (store.getState().vacations.length === 0) {
      fetch("http://localhost:3002/api/vacations")
        .then((res) => res.json())
        .then((vacations) => {
          const action: Action = {
    ...
    // state in ComponentDidMount return undefined so I set Timeout
    setTimeout(() => {
  ...
  private checkFollowedVacations = () => {
    if (
      ...
      fetch(
        `http://localhost:3002/api/vacations/get-followed-vacations/${this.state.user.userId}`
      )
        .then((res) => res.json())
          ...
          {this.state.vacations.map((v) => (
            <Grid item xs={12} md={4} key={v.vacationId}>
              <VactionBox
                ...
                start_date={this.fixDateFromMySQL(v.start_date)}
                end_date={this.fixDateFromMySQL(v.end_date)}
                price={v.price}
                follow={v.follow}
