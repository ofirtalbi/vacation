...
export class AddVacation extends Component<any, AddVacationState> {
  ...
  private checkForm = async () => {
    ...
    if (vacation.start_date === vacation.end_date) {
      alert("Departing date cannot be the same as returning date !");
      return;
    }
    if (!vacation.image) {
    ...
    if (
      vacation.description === undefined ||
      vacation.destination === undefined ||
      vacation.start_date === undefined ||
      vacation.start_date === null ||
      vacation.end_date === undefined ||
      vacation.end_date === null ||
      vacation.price === undefined
    ) {
  ...
  private sendForm = () => {
    ...
    fetch("http://localhost:3002/api/vacations/new-vacation", options)
      .then((res) => res.json())
      .then((vacation) => {
        const action: Action = {
                    ...
                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                      <Grid container justify="space-around">
                        <KeyboardDatePicker
                          margin="normal"
                          format="dd/MM/yyyy"
                          value={this.state.vacation.start_date}
                    ...
                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                      <Grid container justify="space-around">
                        <KeyboardDatePicker
                          margin="normal"
                          format="dd/MM/yyyy"
                          value={this.state.vacation.end_date}
